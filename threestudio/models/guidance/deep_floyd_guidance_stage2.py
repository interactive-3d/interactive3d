...
@threestudio.register("deep-floyd-guidance-sr")
class DeepFloydGuidanceSR(BaseObject):
    @dataclass
    class Config(BaseObject.Config):
        pretrained_model_name_or_path: str = "DeepFloyd/IF-II-L-v1.0"
    ...
    def configure(self) -> None:
        ...
        # Create model
        self.pipe = IFSuperResolutionPipeline.from_pretrained(
        ...
        threestudio.info(f"Loaded Deep Floyd Stage II!")
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # generator = torch.manual_seed(1).cuda()
        generator = None
        noise_level = 250 
        ...
        assert org_imgs is not None
        org_BCHW = org_imgs.permute(0, 3, 1, 2)
        org_BCHW = org_BCHW * 2.0 - 1.0

        cond_image = F.interpolate(
            org_BCHW, (64, 64), mode="bilinear", align_corners=False
        )
        if rgb_BCHW.shape[-1] != 256:
        ...
        upscaled = F.interpolate(cond_image, (256, 256), mode="bilinear", align_corners=True)
        ...
        if prompt_utils.use_perp_neg:
            noise_level = torch.cat([noise_level] * 4) # classifier free guidance
            (
                text_embeddings,
            ...
            with torch.no_grad():
                ...
                upscaled = torch.cat([upscaled]*4, dim=0)
                latent_model_input = torch.cat([latent_model_input, upscaled], dim=1)
            ...
            noise_level = torch.cat([noise_level] * 2) # classifier free guidance
            ...
            with torch.no_grad():
                ...
                upscaled = torch.cat([upscaled]*2, dim=0)
                latent_model_input = torch.cat([latent_model_input, upscaled], dim=1)
                # pred noise
                noise_pred = self.forward_unet(
                    latent_model_input,
        ...
        # visualize the pred x0
        if self.cnt % 50 == 0:
            # compute the previous noisy sample x_t -> x_t-1
            noise_pred_cat = torch.cat([noise_pred, predicted_variance], dim=1)
            pred_x0 = self.scheduler.step(
                noise_pred_cat, t[0].item(), latents_noisy, return_dict=True
            )['pred_original_sample']

            # save image
            # from torchvision.utils import save_image
            # save_imgs = torch.cat([upscaled[0:1], latents, latents_noisy, pred_x0], dim=0)
            # save_imgs = (save_imgs + 1)/2.
            # save_image(save_imgs, 'debug_data/sr.png', nrow=4)
        self.cnt += 1
