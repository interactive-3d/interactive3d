...
@dataclass
class EditRandomCameraDataModuleConfig:
...
class EditRandomCameraIterableDataset(IterableDataset, Updateable):
    def __init__(self, cfg: Any) -> None:
        super().__init__()
        self.cfg: EditRandomCameraDataModuleConfig = cfg
...
@dataclass
class EditRandomMultiviewCameraDataModuleConfig(EditRandomCameraDataModuleConfig):
    relative_radius: bool = True
    n_view: int = 1
    zoom_range: Tuple[float, float] = (1.0, 1.0)


class EditRandomMultiviewCameraIterableDataset(EditRandomCameraIterableDataset):
    ...
    def move_camera(self):
        if not self.edit_mode:
            if self.save_edit_flag:
                ...
                print("=== edit prompt list === ", self.edit_prompt_list, " ==========")
            self.random_edit()
        ...
        # if random.random() < 0.5:
        if random.random() < 1.: # TODO: refactor this block, now simple hacked 
            # sample elevation angles uniformly with a probability 0.5 (biased towards poles)
            elevation_deg = (
...
@register("edit-multiview-camera-datamodule")
class RandomMultiviewCameraDataModule(pl.LightningDataModule):
    cfg: EditRandomMultiviewCameraDataModuleConfig

    def __init__(self, cfg: Optional[Union[dict, DictConfig]] = None) -> None:
        super().__init__()
        self.cfg = parse_structured(EditRandomMultiviewCameraDataModuleConfig, cfg)

    def setup(self, stage=None) -> None:
        if stage in [None, "fit"]:
            self.train_dataset = EditRandomMultiviewCameraIterableDataset(self.cfg)
